                  ...
                  <div class="flex flex-col flex-1 justify-center mb-8">
                      ...
                      <div class="w-full mt-4">
                          <form class="form-horizontal w-3/4 mx-auto" @submit.prevent="onSubmit">
                              ...
                              <div class="flex flex-col mt-4">
                                  <input v-model="confirmPassword" id="confirmPassword" type="password" class="flex-grow h-8 px-2 rounded border border-grey-400" name="password" required placeholder="Confirm Password">
                              </div>
    ...
    function onSubmit() {
      if (email.value && password.value) {
        if (password.value !== confirmPassword.value) {
          hasErrors.value = true
          message.value = 'Your passwords do not match.'
          return
        }
        createAccount(email.value, password.value).then(async data => {
          if (data) {
